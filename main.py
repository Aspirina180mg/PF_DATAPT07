...
st.set_page_config(page_title="PTF Subway ML", layout="wide")
...
try:
    ...
    muestra = X_subway.query(
        f"id_restaurante == '0x87a71b04e42e4c6d:0xdc11ec9338940205'"
    )


calificacion = {0: "😢 Mala", 1: "😀 Buena"}

calificacion_restaurant = calificacion[
    modelo.predict(X_subway_proc.loc[muestra.index])[0]
]
    ...
    if submitted:
        ...
        for i in range(69):
            if X_subway_proc.iloc[muestra.index, i].values[0] != df.iloc[0, i]:
                ...
                features.append(
                    (
                        nombres_atributo[X_subway_proc.columns[i].split("_")[0]],
                        X_subway_proc.columns[i].split("_")[1],
                        cambio[df.iloc[0, i]],
                    )
                )
    ...
    for i in list(set_feats):
        ...
        for j in feats:
            if 3 <= len(feats[j]) <= 5:
                ...
                for f in feats[j]:
                    sugerencia = (
                        f"{nombres_atributo[f.split('_')[0]]}: {f.split('_')[1]}"
                    )
